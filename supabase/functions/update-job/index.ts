...
Deno.serve(async (request) => {
  ...
  if (!jobId || !isUuid(jobId)) return jsonResponse({ error: "A valid job is required." }, 400);
  ...
  if (!supabaseUrl || !serviceRoleKey) return jsonResponse({ error: "Job editing is not ready yet." }, 500);
  ...
  if (!existingJob) {
    return jsonResponse({ error: "This job could not be found." }, 404);
  }
  ...
  // Deliberately not touching status/published_at here - editing details
  // is a separate action from pausing/closing/publishing a job, which
  // already have their own controls (Pause/Close on the jobs list,
  // publish_job at creation time).
  ...
  if (updateError) {
    console.error("update-job update failed", {
      employerProfileId,
      jobId,
      code: updateError.code,
      message: updateError.message
    });
    return jsonResponse({ error: "We couldn't save these changes. Please check the form and try again." }, 500);
  }
  ...
  if (uniqueCategories.length > 1) {
    ...
    if (detailError) {
      console.error("update-job category detail upsert failed", {
        jobId: job.id,
    ...
    // Clear any stale multi-category detail from a previous edit that had
    // more than one category selected - otherwise dropping back down to a
    // single category here would leave the old, now-inaccurate list behind.
    await client.from("job_category_details").delete().eq("job_id", job.id);
  }
